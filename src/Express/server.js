...
const mongoDBDataQueryFunc = async (queryFilter) => {
    ...
    try {
        ...
        console.log("[Express server.js] Found data: " + JSON.stringify(findResult));
        console.log("[TimeStamp] " + Date.now());
    } catch (err) {
        throw (err);
    } finally {
...
app.listen(process.env.SERVER_PORT, process.env.SERVER_IP_ADDR, () => {
    console.log(`[Express server.js] Listening on address ${process.env.SERVER_IP_ADDR} with port ${process.env.SERVER_PORT}`);
    console.log("[Express server.js] Will be connecting to MongoDB with URI: " + process.env.MONGODB_URI);
});
